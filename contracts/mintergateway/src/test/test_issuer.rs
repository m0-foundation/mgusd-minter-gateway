...
fn test_frozen_user_cannot_send_to_issuer() {
    ...
    s.contract.unfreeze_account(&s.distributor, &other);
...
fn test_freeze_issuer_panics_no_trustline() {
    ...
    let result = s.contract.try_freeze_account(&s.distributor, issuer);
    assert!(
        result.is_err(),
...
fn test_unfreeze_issuer_panics_no_trustline() {
    ...
    let result = s.contract.try_unfreeze_account(&s.distributor, issuer);
    assert!(
        result.is_err(),
...
fn test_issuer_cannot_be_frozen_to_block_send_to_issuer() {
    ...
    let freeze_result = s.contract.try_freeze_account(&s.distributor, issuer);
...
fn test_operations_work_after_failed_issuer_freeze() {
    ...
    let _ = s.contract.try_freeze_account(&s.distributor, issuer);
