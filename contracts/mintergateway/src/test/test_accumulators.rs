...
// =============================================================================
// PRINCIPAL PRESENT-VALUE TESTS (TDD RED — these FAIL against current code)
//
// total_principal must store present-value amounts:
//   on mint:  principal += amount * INDEX_SCALE / latest_index
//   on burn:  principal -= amount * INDEX_SCALE / latest_index
//
// The current code adds/subtracts the raw nominal amount, which causes yield
// overestimation after mint and underestimation after burn whenever the index
// has grown above INDEX_SCALE.
// =============================================================================
...
fn test_mint_after_index_growth_stores_present_value_principal() {
    ...
    let pv_of_mint2 = (one_million as u128) * INDEX_SCALE / index_at_mint2;
    let expected_principal = one_million as u128 + pv_of_mint2;

    assert_eq!(
        s.contract.total_principal() as u128,
        expected_principal,
        ...
        expected_principal,
        s.contract.total_principal() as u128 - expected_principal
    );
}
...
fn test_yield_overestimation_after_mint_at_grown_index() {
    ...
    let pv_of_mint2 = (one_million as u128) * INDEX_SCALE / index_yr1;
    let correct_principal_yr2 = one_million as u128 + pv_of_mint2;
    ...
    let expected_total = yield_after_yr1 as u128 + correct_yield_yr2;

    assert_eq!(
        total_claimed as u128, expected_total,
...
fn test_burn_after_index_growth_stores_present_value_principal() {
    ...
    let pv_of_burn = (burn_amount as u128) * INDEX_SCALE / index_at_burn;
    let expected_principal = two_million as u128 - pv_of_burn;
    ...
    assert_eq!(
        ...
        s.contract.total_principal(),
        expected_principal,
        expected_principal - s.contract.total_principal() as u128
    );
}
...
fn test_yield_underestimation_after_burn_at_grown_index() {
    ...
    let pv_of_burn = (burn_amount as u128) * INDEX_SCALE / index_yr1;
    let correct_principal_yr2 = two_million as u128 - pv_of_burn;
    ...
    assert_eq!(
        ...
        total_claimed,
        expected_total,
        expected_total - total_claimed as u128
    );
}
...
fn test_large_index_growth_amplifies_principal_error() {
    ...
    // The gap should be ~260K tokens (14.9% overstatement)
    let actual_principal = s.contract.total_principal() as u128;
    let overstatement = actual_principal - expected_principal;
    ...
    assert_eq!(
        ...
        expected_principal,
        overstatement,
        (overstatement as f64 / expected_principal as f64) * 100.0
    );
}
...
fn test_sequential_mints_at_different_indices_accumulate_pv() {
    ...
    let pv_mint1 = one_million as u128; // INDEX_SCALE / INDEX_SCALE = 1
    let pv_mint2 = (one_million as u128) * INDEX_SCALE / index_yr1;
    let pv_mint3 = (one_million as u128) * INDEX_SCALE / index_yr2;
