...
fn test_sac_transfer_multiple_recipients() {
    ...
    s.contract.onboard_user(&treasury, &s.onboarder);
    s.contract.onboard_user(&recipient_a, &s.onboarder);
    s.contract.onboard_user(&recipient_b, &s.onboarder);
...
fn test_onboarding_flow() {
    ...
    s.contract.onboard_user(&new_user, &s.onboarder);
    s.contract.onboard_user(&existing_user, &s.onboarder);
