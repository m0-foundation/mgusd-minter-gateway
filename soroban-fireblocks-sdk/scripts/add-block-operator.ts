/**
 * Add a block operator to the wrapper contract.
 *
 * Edit the VARS block below for this specific execution, then run:
 *   npm run add-block-operator
 *
 ...
 * before we sign anything. Idempotent: if OPERATOR_TO_ADD is already a block
 * operator, the contract no-ops (no event emitted).
 *
...
// New block operator to grant block permission to (Predicate).
  ...
  if (onChainAdmin !== vaultPubkey) {
    throw new Error(
      `Admin mismatch: on-chain admin is ${onChainAdmin}, but VAULT_PUBLIC_KEY is ${vaultPubkey}. ` +
        `Only the admin can add a block operator. Fix the VARS block or rotate the admin first.`,
    );
  }

  const alreadyOperator = await client.queryIsBlockOperator({ contractId, account: newOperator });

  console.log("=== Add block operator ===");
  console.log(`  Contract:     ${contractId}`);
  console.log(`  Admin:        ${vaultPubkey} (vault ${VAULT_ACCOUNT_ID})`);
  console.log(`  New operator: ${newOperator}`);
  console.log(`  Currently a block operator? ${alreadyOperator ? "YES (call will be a no-op)" : "no"}`);
  console.log();

  if (alreadyOperator) {
    console.log("Already a block operator — nothing to do. Exiting.");
    return;
  }

  if (!(await confirm(`Grant block permission on ${contractId} to ${newOperator}?`))) {
    console.log("Aborted.");
    return;
  }

  const result = await client.addBlockOperator({ contractId, addr: newOperator });
  printResult(result);

  const nowOperator = await client.queryIsBlockOperator({ contractId, account: newOperator });
  if (!nowOperator) {
    throw new Error(
      "add_block_operator returned SUCCESS but is_block_operator() still returns false — investigate on-chain",
    );
  }
  console.log(`✓ Verified: ${newOperator} is now a block operator.`);
}
