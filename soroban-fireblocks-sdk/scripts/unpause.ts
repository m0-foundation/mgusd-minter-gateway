...
async function main(): Promise<void> {
  ...
  if (!(await confirm(`UNPAUSE ${contractId}?`))) {
    console.log("Aborted.");
    return;
  }

  const result = await client.unpause({ contractId, caller: config.sourcePublicKey });
  printResult(result);

  const paused = await client.queryPaused({ contractId });
  if (paused) {
    throw new Error("unpause() returned SUCCESS but `paused()` is still true — investigate on-chain");
  }
}
